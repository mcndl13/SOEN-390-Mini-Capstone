...
export default function LocationScreen() {
  ...
  const fetchBuildingName = async (latitude: number, longitude: number): Promise<void> => {
    try {
      let response = await fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&key=${GOOGLE_MAPS_API_KEY}`
      );
      let data: GeocodeResponse = await response.json();
  ...
  return (
    <View style={styles.container}>
      <Button title={`Switch to ${campus === 'SGW' ? 'Loyola' : 'SGW'} Campus`} onPress={toggleCampus} />
      <Text>{currentBuilding ? `You are at: ${currentBuilding}` : 'Fetching building info...'}</Text>
      <MapView
        style={styles.map}
        initialRegion={{
